...
return(sum/count);}});SUGAR.CeilingExpression=function(params){this.init(params);}
SUGAR.util.extend(SUGAR.CeilingExpression,SUGAR.NumericExpression,{className:"CeilingExpression",evaluate:function(){return Math.ceil(this.getParameters().evaluate());},getParamCount:function(){return 1;}});SUGAR.DivideExpression=function(params){this.init(params);}
SUGAR.util.extend(SUGAR.DivideExpression,SUGAR.NumericExpression,{className:"DivideExpression",evaluate:function(){var params=this.getParameters();var numerator=params[0].evaluate();var denominator=params[1].evaluate();return numerator/denominator;},getParamCount:function(){return 2;}});SUGAR.FloorExpression=function(params){this.init(params);}
...
max=val;}
return max;}});SUGAR.MedianExpression=function(params){this.init(params);}
...
return diff;}});SUGAR.ValueOfExpression=function(params){this.init(params);}
...
SUGAR.forms.FlashField(field);}}});SUGAR.forms.SetRequiredAction=function(variable,expr,label){this.variable=variable;this.expr=expr;this.label=label;this._el_lbl=document.getElementById(this.label);}
...
SUGAR.forms.FormValidator.setRequired(el.form.name,el.name,this.required);if(this._el_lbl!=null){var p=this._el_lbl;var els=YAHOO.util.Dom.getElementsBy(function(e){return e.className=='req';},"span",p)
var reqSpan=false;if(els!=null&&els[0]!=null)
reqSpan=els[0];if((this.required==true||this.required=='true')){if(!reqSpan){var node=document.createElement("span");node.innerHTML="<font color='red'>*</font>";node.className="req";this._el_lbl.appendChild(node);addToValidate('EditView',this.variable,'text',true,this.variable);}}else{if(p!=null&&reqSpan!=false){p.removeChild(reqSpan);}
removeFromValidate('EditView',this.variable);}}}});SUGAR.forms.SetValueAction=function(target,valExpr){this.expr=valExpr;this.target=target;};SUGAR.util.extend(SUGAR.forms.SetValueAction,SUGAR.forms.AbstractAction,{exec:function(context)
{if(typeof(context)=='undefined')
...
{if(typeof(this.attrs[i])==='object'){temp[i]=this.evalExpression(this.attrs[i].evaluate,context);}else{temp[i]=this.attrs[i];}}
