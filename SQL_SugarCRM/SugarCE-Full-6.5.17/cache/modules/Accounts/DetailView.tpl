...
{counter name="fieldsUsed"}
<td width='12.5%' scope="col">
{if !$fields.setpr_c.hidden}
{capture name="label" assign="label"}{sugar_translate label='LBL_SETPR' module='Accounts'}{/capture}
{$label|strip_semicolon}:
{/if}
</td>
<td width='37.5%'  >
...
{if is_string($fields.setpr_c.options)}
<input type="hidden" class="sugar_field" id="{$fields.setpr_c.name}" value="{ $fields.setpr_c.options }">
{ $fields.setpr_c.options }
{else}
<input type="hidden" class="sugar_field" id="{$fields.setpr_c.name}" value="{ $fields.setpr_c.value }">
{ $fields.setpr_c.options[$fields.setpr_c.value]}
{/if}
{/if}
</td>
</tr>
